...
{
    public class ContractResourceLeasing : Entity
    {
        ...
        public override IEnumerable<Structures.ExceptionsStruct> SaveToRX(Logger logger, bool supplementEntity, string ignoreDuplicates, int shift = 0)
        {
            ...
            try
            {
                ...
                var contract = BusinessLogic.GetEntityWithFilter<IContractResourceLeasing>(x => x.RegistrationNumber != null && x.RegistrationNumber == regNumber &&
                    x.RegistrationDate == regDateBeginningOfDay &&
                    x.Counterparty.Id == counterparty.Id, exceptionList, logger, true);
                if (contract == null)
                    contract = new IContractResourceLeasing();
                ...
                var createdContract = BusinessLogic.CreateEntity<IContractResourceLeasing>(contract, exceptionList, logger);

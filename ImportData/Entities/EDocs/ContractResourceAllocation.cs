...
{
    public class ContractResourceAllocation : Entity
    {
        ...
        public override IEnumerable<Structures.ExceptionsStruct> SaveToRX(Logger logger, bool supplementEntity, string ignoreDuplicates, int shift = 0)
        {
            ...
            var dataCenter = this.Parameters[shift + 18];

            try
            {
                var regDateBeginningOfDay = BeginningOfDay(regDate.UtcDateTime);
                var contract = BusinessLogic.GetEntityWithFilter<IContractResourceAllocation>(x => x.RegistrationNumber != null && x.RegistrationNumber == regNumber &&
                    x.RegistrationDate == regDateBeginningOfDay &&
                    x.Counterparty.Id == counterparty.Id, exceptionList, logger, true);
                if (contract == null)
                    contract = new IContractResourceAllocation();
                ...
                contract.DataCenter = dataCenter;

                var createdContract = BusinessLogic.CreateEntity<IContractResourceAllocation>(contract, exceptionList, logger);
